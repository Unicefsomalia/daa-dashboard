...
export class DashboardComponent implements OnInit {
  ...
  getMonthlyAttendance() {
    ...
    this.dashboardService.getAttendance('monthly').subscribe(res => {
      ...
      this.att_m_distribution = {
        series: {
          series_a: presents,//.map((v, index) => ({ x: index, y: v })),
          series_b: absents,//.map((v, index) => ({ x: index, y: v })),
          labels: {
            label_a: 'Present',
            label_b: 'Absent'
          }
      ...
      colors: [
        {
          ...
          pointBackgroundColor: '#2AACE2',
          pointBorderColor: '#fff',
          pointHoverBackgroundColor: '#fff',
          pointHoverBorderColor: '#2AACE2'
        },
        {
  ...
  doughnut_configs = {
    colors: ['#2AACE2', '#F5D908']
  }

  attendance_doughnut_configs = {
    colors: ['#2AACE2', '#ff0059e8', '#F5D908', '#e91d63'],
